...
> 语言策略：`code`。正文三语言一致；工具风控代码按 Python 生态实现。
...
def check_tool_call(policy: ToolPolicy, user_approved: bool, call_count: int) -> GuardDecision:
    if policy.risk is ToolRiskLevel.DESTRUCTIVE:
        return GuardDecision.DENY
    if policy.requires_approval or policy.risk is ToolRiskLevel.IRREVERSIBLE_WRITE:
        return GuardDecision.ALLOW if user_approved else GuardDecision.REQUIRE_APPROVAL
    if call_count > 20:

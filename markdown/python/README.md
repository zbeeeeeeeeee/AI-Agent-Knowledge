...
> 4 篇含代码的文档已按 Python 生态重写，其余 13 篇为语言无关的概念/面试内容，与 Java 版一致。
...
| `05-系统设计/01-AI应用系统设计.md` | Java 分层接口（8 处） | `dataclass`（`AiRequest`）+ `Enum`（风险等级）+ `Protocol` 接口（Prompt/工具/网关/RAG/评测） |

## 学习路径

1. `01-LLM基础`：LLM 运行机制 → 大模型结构化输出（Python 服务端校验与分发）
2. `02-Agent`：Agent 核心概念 → 记忆系统
3. `03-RAG`：RAG 基础概念 → 向量索引与向量数据库
4. `04-工程实践`：Workflow/Graph/Loop（LangGraph 实战）→ Loop Engineering → Harness Engineering → 大模型网关
5. `05-系统设计`：AI 应用系统设计（Python 后端落地建议）
6. `06-面试`：六份面试题 + 模拟题库

...
**2022 年，ChatGPT 这类产品刚火时**，主要应用仍是与模型“对话”。模型能基于已有知识回答问题，不能主动调用外部工具，也不能自己完成操作。

当时最重要的方法是 Prompt Engineering：提示词越清晰，回答越稳定。
...
关注点从模型参数、上下文长度和 Prompt 技巧扩展到模型之外的工程环境。
...
LangChain 的 Agent 源码核心并不复杂，多数时候是一个 while 循环。
...
![AI Agent 核心架构](../../../HTML/AI Agent 核心概念：Agent Loop、Plan-and-Execute、A2A、Agentic Workflows、Tools 注册 _ JavaGuide_files/agent-core-arch.jpg)
...
**Tools（工具）**：让 LLM 能真正操作外部世界，比如查数据、调 API、读文件、执行代码。没有工具，Agent 大多只能停留在“给出建议”。
...
![Agent Loop 工作流程](../../../HTML/AI Agent 核心概念：Agent Loop、Plan-and-Execute、A2A、Agentic Workflows、Tools 注册 _ JavaGuide_files/agent-loop-flow.jpg)
...
![MCP 图解](../../../HTML/AI Agent 核心概念：Agent Loop、Plan-and-Execute、A2A、Agentic Workflows、Tools 注册 _ JavaGuide_files/mcp-simple-diagram.jpg)
...
![ReAct-LLM](../../../HTML/AI Agent 核心概念：Agent Loop、Plan-and-Execute、A2A、Agentic Workflows、Tools 注册 _ JavaGuide_files/ReAct-LLM.jpg)
...
![ReAct 模式流程](../../../HTML/AI Agent 核心概念：Agent Loop、Plan-and-Execute、A2A、Agentic Workflows、Tools 注册 _ JavaGuide_files/agent-react-flow.jpg)
...
![Multi-Agent 系统架构](../../../HTML/AI Agent 核心概念：Agent Loop、Plan-and-Execute、A2A、Agentic Workflows、Tools 注册 _ JavaGuide_files/agent-multi-agent-arch.jpg)
...
![A2A 协议架构](../../../HTML/AI Agent 核心概念：Agent Loop、Plan-and-Execute、A2A、Agentic Workflows、Tools 注册 _ JavaGuide_files/agent-a2a.jpg)
...
![智能体工作流核心模式](../../../HTML/AI Agent 核心概念：Agent Loop、Plan-and-Execute、A2A、Agentic Workflows、Tools 注册 _ JavaGuide_files/agent-agentic-workflows.jpg)
...
常见误解是 Agent 必须由模型自行规划、调用工具并跑完全程；实际落地中，这种全自主模式稳定性不足。

纯 Agent 里，LLM 是决策者。每一步是否调工具、调哪个工具、下一步怎么走，主要靠模型推理；给定任务后，模型自行尝试跑完。

AI 工作流里，LLM 只是流程中的一个节点。步骤顺序、条件跳转、失败重试等流程骨架提前设计，控制权在图结构中，不在模型手里。
...
常见认知偏差是把任务“路径不确定”当作需求未拆清。认真拆分后，多数场景可归为“LLM 在固定节点做生成或判断”，此类场景用 Workflow 更稳、更易排查。

真正适合纯 Agent 的任务，是无法预先写出执行步骤的场景。比如“帮我排查这个线上故障”，查什么、怎么查、查到什么程度，很难事先规定死。

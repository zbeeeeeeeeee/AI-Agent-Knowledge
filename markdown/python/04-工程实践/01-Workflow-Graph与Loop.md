...
# LangGraph 用 TypedDict 定义状态，用 Annotated[type, reducer] 声明更新策略
...
class WorkflowState(TypedDict, total=False):
    ...
    current_draft: str                          # 当前草稿（覆盖）
    review_score: float                         # 审核评分（覆盖）
    review_feedback: str                        # 审核反馈（覆盖）
    iteration_count: int                        # 迭代计数（覆盖）
    output: str                                 # 最终输出（覆盖）
    next_node: str                              # 路由控制（覆盖）
```
...
def review_node(state: WorkflowState, llm) -> dict:
    ...
    # 解析评分和反馈（实际项目中使用结构化输出）
    score = parse_score(response)
    feedback = parse_feedback(response)

    next_node = "exit" if (score >= 80 or count >= 3) else "revise"
    return {
        "review_score": score,
...
def exit_node(state: WorkflowState) -> dict:
    return {"output": state.get("current_draft", "")}
```
...
def build_workflow(llm):
    ...
    workflow.add_node("draft", lambda s: draft_node(s, llm))
    workflow.add_node("review", lambda s: review_node(s, llm))
    workflow.add_node("revise", lambda s: revise_node(s, llm))
    ...
    workflow.add_conditional_edges("draft", lambda s: s.get("next_node", "review"),
                                   {"review": "review"})
    workflow.add_conditional_edges("review", lambda s: s.get("next_node", "exit"),
                                   {"revise": "revise", "exit": "exit"})

    # 修改后回到审核节点，形成循环
    workflow.add_conditional_edges("revise", lambda s: s.get("next_node", "review"),
                                   {"review": "review"})
...
> 这些模式需要在节点内部或中间件层自行实现，Graph 框架只提供执行骨架和状态管理。具体做法：重试和熔断逻辑封装在节点里，通过 State 字段（如 `retry_count`、`circuit_state`）持久化状态；舱壁隔离用 Python 的 `asyncio.Semaphore` 或 `limits` 库；补偿事务需要在 State 中记录已完成步骤的回滚信息，再设计专门的补偿节点。

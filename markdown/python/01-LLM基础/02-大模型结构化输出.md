...
模型可能因把握不足省略 `priority`，或因认为 `confidence` 不重要而省略。后端 DTO 反序列化、规则引擎、数据库写入不会因为模型“没把握”就自动补齐。
...
JSON 语法是合法的，但业务类型不合法。`needManualReview` 是字符串，不是布尔值；`confidence` 是字符串，不是数字。部分系统在反序列化时自动转换类型，会静默吞掉上游错误，增加排查成本。
...
这类文本适合人工阅读，不适合程序解析。结构化输出场景里，**可读性不是第一目标，可解析性才是第一目标**。
...
Anthropic 官方文档说明：Claude 会根据用户请求和工具描述决定是否调用工具，并返回结构化调用；客户端工具由应用执行，再将 `tool_result` 发回。Gemini 官方文档也强调，Function Calling 会让模型决定要调用哪个函数并提供参数，真正调用实际函数的动作在应用侧完成。
...
后端将 `urgent` 映射为 `HIGH` 还是判为非法值？服务端做模糊映射，等于把模型的不确定性扩散到业务规则中。
...
## Python 后端示例：把订单查询做成可校验工具

下面用一个订单查询工具做完整示例。场景是：用户用自然语言询问订单状态，模型通过 Function Calling 生成 `query_order` 工具调用，Python 服务端校验参数后分发到订单服务。
...
下面示例使用标准库 `json` 解析 JSON，使用 `jsonschema` 做结构校验。真实项目中，依赖版本建议用 `requirements.txt` / `pyproject.toml` 锁定，并定期做安全扫描。

...
> 本文回答：什么时候才需要多智能体？有哪些编排模式？Agent 之间怎么通信？为什么多数生产系统不应该一上来就上多 Agent？

## 一、先问：真的需要多智能体吗

多智能体不是单 Agent 的“高级替代品”，而是用**通信成本、调试成本、Token 成本**换取**并行度与专业分工**。

在决定上多智能体前，先确认三件事：

1. 单 Agent 的失败，是因为“上下文太杂”，还是任务天然需要多个角色？
2. 任务是否可以被稳定拆成多个互不冲突的子任务？
3. 团队是否有能力观测和调试多 Agent 之间的消息流？

如果只是上下文杂乱，优先做 Context Engineering、Memory 分层或 Skill 化；如果任务可稳定拆解，优先用 Workflow / Graph，而不是多 Agent。

**适合多智能体的信号**：

- 子任务需要不同的 System Prompt、工具集或记忆边界；
- 子任务可并行，且互不阻塞；
- 需要一个 Agent 生成、另一个 Agent 审查的对抗式质量结构；
...
## 七、最小实现示例：Orchestrator-Workers（伪代码）

...
> 语言策略：`code`。正文三语言一致；检索融合与重排代码按 Java 生态实现。
...
```java
public record ScoredDoc(String docId, double score, String source) {}

public class RRF {
    public static final double K = 60.0;

    public static List<ScoredDoc> fuse(List<List<ScoredDoc>> rankedLists, int topN) {
        Map<String, Double> rrf = new LinkedHashMap<>();
        Map<String, String> sources = new LinkedHashMap<>();
        for (List<ScoredDoc> list : rankedLists) {
            for (int i = 0; i < list.size(); i++) {
                ScoredDoc doc = list.get(i);
                rrf.merge(doc.docId(), 1.0 / (K + i + 1), Double::sum);
                sources.putIfAbsent(doc.docId(), doc.source());
            }
        }
        return rrf.entrySet().stream()
            .sorted(Map.Entry.<String, Double>comparingByValue().reversed())
            .limit(topN)
            .map(e -> new ScoredDoc(e.getKey(), e.getValue(), sources.get(e.getKey())))
            .toList();
    }
}
```

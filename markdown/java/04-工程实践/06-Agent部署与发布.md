...
> 语言策略：`code`。正文三语言一致；服务骨架与容器化示例按 Java 生态实现。
...
```java
public record AgentRunRequest(String taskId, String input, String promptVersion, String userId) {}
public record AgentRunResponse(String taskId, String status, String result, long elapsedMs) {}
public record HealthStatus(String status, boolean modelUp, boolean vectorDbUp, boolean toolsUp) {}
...
public class AgentService implements AgentRuntime {
    ...
    public AgentService(AgentRuntime delegate) { this.delegate = delegate; }

    @Override
    public AgentRunResponse run(AgentRunRequest request) {
        // 生产实现：先做幂等校验，再进入 Agent Loop，超时或异常返回可重试状态

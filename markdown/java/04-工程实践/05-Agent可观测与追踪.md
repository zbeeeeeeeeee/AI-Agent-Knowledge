...
1. 普通服务监控只回答“接口慢不慢”，Agent 可观测还需回答“为什么这么走”。
...
```java
public record AgentSpan(String traceId, String taskId, String spanId, String parentSpanId,
                        String node, String model, String promptVersion, String toolName,
                        String status, long latencyMs, long inputTokens, long outputTokens) {}

public class AgentTelemetry {
    private final List<AgentSpan> spans = new ArrayList<>();

    public void record(AgentSpan span) { spans.add(span); }

    public long countTasks() { return spans.stream().map(AgentSpan::taskId).distinct().count(); }

    public double toolErrorRate() {
        long toolSpans = spans.stream().filter(s -> s.toolName() != null).count();
        long toolErrors = spans.stream()
            .filter(s -> s.toolName() != null && "error".equals(s.status())).count();
        return toolSpans == 0 ? 0 : (double) toolErrors / toolSpans;
    }

    public long totalTokens() {
        return spans.stream().mapToLong(s -> s.inputTokens() + s.outputTokens()).sum();
    }
}
```

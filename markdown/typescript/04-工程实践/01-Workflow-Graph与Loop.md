...
单轮对话能回答问题，但很难稳定地**交付结果**。线上真实任务很少是“问一句答一句”：检索信息、调用工具、输出结构化结果、校验格式、失败重试、不满意再来一轮，这些步骤串起来才是交付。靠一段超长 Prompt 塞入所有逻辑不可维护，需要**可分支、可循环、可观测**的执行路径。
...
以文章写作为例：一次生成的结果往往不够理想。手动复制结果并追加新要求继续提问，既不高效，也会快速消耗上下文。将这一过程结构化为“**审查 → 修改 → 再审查**”循环，并设定停止条件（如达到质量标准或触达迭代上限），稳定性会明显提升。

说到底，工作流就是把一次性的生成过程，变成一个**可迭代、可收敛、可控制**的系统化流程。

## 传统工作流和 AI 工作流有什么区别？

![传统 Workflow 与 AI Workflow 对比](../../../HTML/AI 工作流中的 Workflow、Graph 与 Loop：从概念到实现 _ JavaGuide_files/traditional-vs-ai-workflow.svg)
...
到了 AI 场景，“流程”一词的含义不同。传统工作流强调顺序性与确定性；AI 工作流处理的是充满不确定性的执行环境，不再只是“按步骤执行”，还包括：
...
AI Workflow 与传统 Workflow 都有流程，差别在于前者更强调动态决策和状态驱动。表达“下一步不唯一”或“不满意就再来一轮”时，线性列表不够用，需要落到 Graph（结构）与 Loop（回溯）两类概念上。

## Graph 和 Loop 是什么？

### Graph：工作流的结构

沿用案例：搭建「生成初稿 → 质量审核 → 不达标则修改 → 再回到审核」路径。每一步对应图的 **Node**，步骤之间的走向由 **Edge** 表达，整条链路读写的共享上下文就是 **State**。
...
只看 Node 和 Edge，只能得到一张“能跑起来的路径图”；加上 State，这张图才能在运行时做决策。
...
`Loop` 常被理解为“多跑几次”，但准确含义是**图结构上的一种控制模式**：某条边根据当前状态把控制流送回先前节点时，就形成 Loop。如上图所示，循环内部由 LLM 按提示词对结果评分，满足条件则输出，否则打回重写。
...
AI 场景里，第二类通常更有代表性：“跑几次”往往不是先验确定的，而是由内容质量、工具执行结果、外部反馈共同决定。实际开发中两者必须同时使用：LLM 的不确定性可能导致内容一直不合格，此时需要按固定次数循环进行降级兜底。
...
![Workflow、Graph、Loop 三者关系概览](../../../HTML/AI 工作流中的 Workflow、Graph 与 Loop：从概念到实现 _ JavaGuide_files/workflow-graph-loop-relation.svg)

可以用一句话收束三者的层次关系：**Workflow 是目标与过程，Graph 是结构与载体，Loop 是图上的控制模式。**

继续沿用同一个“写文章并审核”的例子：

- “先生成初稿，再审核，不达标就修改，直到达标后输出”描述的是 **Workflow**。
- 把 `生成节点 → 检查节点 → 修正节点` 画成节点与连线，并共享同一份状态，得到的是 **Graph**。
- 规定“审核不通过就回到修改，直到评分达标或达到上限”，得到的是 **Loop**。

这三者是同一件事的三个观察角度：Workflow 关注任务目标，Graph 关注结构组织，Loop 关注回溯控制。

## 代码实现

框架映射：以下以 LangGraph.js（TypeScript 生态）为例（Java 生态的 Spring AI Alibaba、Python 生态的 LangGraph 概念一一对应，可对照阅读）。
...
### 实现示例：用 LangGraph.js 构建文章审核工作流

实现：LangGraph.js，构建“生成 → 审核 → 修改”工作流。

**第一步：定义状态和更新策略**

```typescript
// 配置状态键更新策略：控制每个字段如何更新
// 覆盖（Replace）：新值直接替换旧值；追加（Append）：新值追加到已有列表
// LangGraph.js 用 Annotation.Root 定义状态，用 reducer 声明更新策略
import { Annotation } from "@langchain/langgraph";

const WorkflowStateAnnotation = Annotation.Root({
  input: Annotation<string>({ reducer: (_, next) => next }),                    // 用户输入（覆盖）
  messages: Annotation<string[]>({ reducer: (prev, next) => [...prev, ...next] }), // 对话历史（追加）
  current_draft: Annotation<string>({ reducer: (_, next) => next }),            // 当前草稿（覆盖）
  review_score: Annotation<number>({ reducer: (_, next) => next }),             // 审核评分（覆盖）
  review_feedback: Annotation<string>({ reducer: (_, next) => next }),          // 审核反馈（覆盖）
  iteration_count: Annotation<number>({ reducer: (_, next) => next }),          // 迭代计数（覆盖）
  output: Annotation<string>({ reducer: (_, next) => next }),                   // 最终输出（覆盖）
  next_node: Annotation<string>({ reducer: (_, next) => next }),                // 路由控制（覆盖）
});
...
// 生成初稿节点
async function draftNode(state: WorkflowState, llm: LLM): Promise<Partial<WorkflowState>> {
  const draft = await llm.invoke(`请根据以下要求撰写文章：${state.input ?? ""}`);
  return { current_draft: draft, next_node: "review" };
}

// 质量审核节点
async function reviewNode(state: WorkflowState, llm: LLM): Promise<Partial<WorkflowState>> {
  const draft = state.current_draft ?? "";
  const count = state.iteration_count ?? 0;
  ...
  // 解析评分和反馈（实际项目中使用结构化输出 + JSON.parse）
  const score = parseScore(response);
  const feedback = parseFeedback(response);

  const nextNode = score >= 80 || count >= 3 ? "exit" : "revise";
  return {
    review_score: score,
...
// 修改节点：根据审核反馈修正内容
async function reviseNode(state: WorkflowState, llm: LLM): Promise<Partial<WorkflowState>> {
  const draft = state.current_draft ?? "";
  const feedback = state.review_feedback ?? "";
  const revised = await llm.invoke(`请根据反馈修改文章。\n\n原文：${draft}\n\n反馈意见：${feedback}`);
  return { current_draft: revised, next_node: "review" };
}

// 输出节点
async function exitNode(state: WorkflowState): Promise<Partial<WorkflowState>> {
  return { output: state.current_draft ?? "" };
}

function parseScore(response: string): number {
  // 从模型输出中解析评分（实际项目建议用结构化输出）
  return Number(JSON.parse(response).score ?? 0);
}

function parseFeedback(response: string): string {
  return String(JSON.parse(response).feedback ?? "");
}
```
...
function buildWorkflow(llm: LLM) {
  const workflow = new StateGraph(WorkflowStateAnnotation);

  workflow.addNode("draft", async (state) => draftNode(state, llm));
  workflow.addNode("review", async (state) => reviewNode(state, llm));
  workflow.addNode("revise", async (state) => reviseNode(state, llm));
  ...
  workflow.addConditionalEdges("draft", (state) => state.next_node ?? "review", {
    review: "review",
  });
  workflow.addConditionalEdges("review", (state) => state.next_node ?? "exit", {
    revise: "revise", // 审核不通过 → 修改
    exit: "exit",     // 审核通过或达到上限 → 输出
  });

  // 修改后回到审核节点，形成循环
  workflow.addConditionalEdges("revise", (state) => state.next_node ?? "review", {
    review: "review",
  });
  workflow.addEdge("exit", END);

  // 配置持久化：生产环境建议使用 RedisSaver 或 SqliteSaver
...
> 更完整的示例（包括人机协同、持久化、流式输出）可参考 [LangGraph.js 官方文档](https://langchain-ai.github.io/langgraphjs/)。
...
![Graph 核心元素：Node、Edge、State](../../../HTML/AI 工作流中的 Workflow、Graph 与 Loop：从概念到实现 _ JavaGuide_files/graph-core-elements.svg)
...
主流 Graph 框架（如 Spring AI Alibaba、LangGraph）把错误分成四类，对应不同处理策略：
...
> 这些模式需要在节点内部或中间件层自行实现，Graph 框架只提供执行骨架和状态管理。具体做法：重试和熔断逻辑封装在节点里，通过 State 字段（如 `retry_count`、`circuit_state`）持久化状态；舱壁隔离用 TypeScript 的 `p-limit` 或 `opossum` 库；补偿事务需要在 State 中记录已完成步骤的回滚信息，再设计专门的补偿节点。

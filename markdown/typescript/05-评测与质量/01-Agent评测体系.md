...
3. 没有回归门禁的 Agent 系统，Prompt、工具或模型一改就会悄悄退化。回归门禁必须可执行。
...
interface EvalCase {
  taskId: string;
  difficulty: "easy" | "medium" | "hard" | "negative";
  input: string;
  expectedTool?: string;
  acceptance: string;
...
function evaluate(c: EvalCase, run: AgentRun): EvalResult {
  const success = run.finalOutcome.includes(c.acceptance);
  const toolHit = !c.expectedTool || run.toolCalls.includes(c.expectedTool);
  const forbidden = !!c.forbiddenTool && run.toolCalls.includes(c.forbiddenTool);
  return {
    taskId: c.taskId,
    success,
    toolHit,
    forbiddenToolUsed: forbidden,
    steps: run.toolCalls.length,
    latencyMs: run.latencyMs,
  };
}

function passGate(results: EvalResult[], threshold = 0.9): boolean {
  if (results.length === 0) return false;
  const success = results.filter(r => r.success).length / results.length;
  const toolHit = results.filter(r => r.toolHit).length / results.length;
  const noForbidden = results.every(r => !r.forbiddenToolUsed);
  return noForbidden && success >= threshold && toolHit >= threshold;
}
```

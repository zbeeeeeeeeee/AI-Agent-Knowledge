...
![生成阶段三层约束：JSON Mode 管语法，JSON Schema 管契约，Structured Outputs 把契约前移到模型生成阶段](../../HTML/大模型结构化输出：从 JSON 契约到 Function Calling 落地 _ JavaGuide_files/structured-output-function-calling-three-layer-constraint.png)

一句话：**JSON Mode 管语法，JSON Schema 管契约，Structured Outputs 把契约前移到模型生成阶段；但无论模型侧约束多强，服务端校验都不能省**。

![流程图](../../HTML/_mermaid/大模型结构化输出：从 JSON 契约到 Function Calling 落地 _ JavaGuide_files/mermaid-v-6.svg)
...
![Function Calling 完整调用链路：模型只生成调用意图，真正执行工具的是业务侧](../../HTML/大模型结构化输出：从 JSON 契约到 Function Calling 落地 _ JavaGuide_files/structured-output-function-calling-function-calling-pipeline.png)
...
![流程图](../../HTML/_mermaid/大模型结构化输出：从 JSON 契约到 Function Calling 落地 _ JavaGuide_files/mermaid-v-5.svg)
...
![流程图](../../HTML/_mermaid/大模型结构化输出：从 JSON 契约到 Function Calling 落地 _ JavaGuide_files/mermaid-v-4.svg)
...
![流程图](../../HTML/_mermaid/大模型结构化输出：从 JSON 契约到 Function Calling 落地 _ JavaGuide_files/mermaid-v-3.svg)
...
![工具调用安全风险分层：按风险等级匹配不同的控制策略](../../HTML/大模型结构化输出：从 JSON 契约到 Function Calling 落地 _ JavaGuide_files/structured-output-function-calling-tool-call-security.png)

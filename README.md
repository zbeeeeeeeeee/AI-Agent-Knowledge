...
> 当前版本：**知识地图分层索引版 v0.9**（工程主题精简 + 表达去口语化）。

## 快速导航

| 入口 | 作用 |
| --- | --- |
| [知识地图总索引.md](知识地图总索引.md) | 🧭 L1 分类入口：8 个分类卡片 + 每个分类第一入口 + 两条常用路径 |
| [知识地图-快速查找.md](知识地图-快速查找.md) | L2 快速查找：按模块、按任务、按角色三张表 |
| [知识地图-全量节点索引.md](知识地图-全量节点索引.md) | L3 全量附录：176 节点状态 + 26×3 语言矩阵 |
| [Agent知识地图.xmind](Agent知识地图.xmind) | 视觉层：XMind 版脑图，可直接打开评审 |
...
| [STYLE.md](STYLE.md) | 开发者文档写作约定：写作前先看 |
...
3. `知识地图总索引.md` 的可点击索引提供 **Java / Python / TypeScript 三列直达链接**，并附 **26 篇 × 3 语言覆盖矩阵**。
...
- 地图覆盖：176 个叶子节点，🟢 140 / 🟡 32 / 🔴 4，加权覆盖率约 **89%**。
- 板块覆盖：架构 97%、工程化 97%、生态前沿 89%、评测 88%、概念 85%、安全 83%、应用案例 75%。

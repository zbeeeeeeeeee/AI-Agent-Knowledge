...
\section*{一、先问：真的需要多智能体吗}


多智能体不是单 Agent 的“高级替代品”，而是用\textbf{通信成本、调试成本、Token 成本}换取\textbf{并行度与专业分工}。
...
\begin{enumerate}[itemsep=1pt, topsep=2pt]
  \item 单 Agent 的失败，是因为“上下文太杂”，还是任务天然需要多个角色？
  \item 任务是否可以被稳定拆成多个互不冲突的子任务？
  \item 团队是否有能力观测和调试多 Agent 之间的消息流？
\end{enumerate}
...
\begin{itemize}[itemsep=1pt, topsep=2pt]
  ...
  \item 子任务可并行，且互不阻塞；
  \item 需要一个 Agent 生成、另一个 Agent 审查的对抗式质量结构；
  \item 不同角色由不同团队维护，需要独立发布与权限隔离。
\end{itemize}


\section*{二、五种核心编排模式 + 一种特殊模式}
...
\section*{四、通信设计：多 Agent 真正的成本在这里}
...
\section*{七、最小实现示例：Orchestrator-Workers（伪代码）}

...
\begin{quote}
语言策略：\texttt{code}。正文三语言一致；上下文组装器按 TypeScript 生态实现。 地图节点：\texttt{04 开发与工程化 / 上下文工程}。配套文档：\href{03-Harness工程.md}{Harness Engineering}、\href{../02-Agent/02-Agent记忆系统.md}{Agent 记忆系统}。
\end{quote}
...
class ContextAssembler {
  static available(budget: ContextBudget): number {
    return budget.windowTokens - budget.reservedOutput
      - budget.systemPrompt - budget.toolSchemas - budget.safetyReserve;
  }

  static assemble(budget: ContextBudget, sections: ContextSection[], maxTokens: number): string {
    const cap = Math.min(maxTokens, this.available(budget));
    const ordered = [...sections].sort((a, b) => a.priority - b.priority);
    const parts: string[] = [];
    let used = 0;

    for (const section of ordered) {
      if (used + section.tokens > cap && section.priority > 1) continue;

...
interface ProviderClient {
  ...
  chat(request: LLMRequest, prompt: RenderedPrompt, route: ModelRoute): Promise<LLMResponse>;
  streamChat(request: LLMRequest, prompt: RenderedPrompt, route: ModelRoute): AsyncIterable<LLMChunk>;
}
...
class RuleBasedModelRouter {
  constructor(
    private readonly routeConfigRepository: RouteConfigRepository,
    private readonly modelHealthService: ModelHealthService,
  ) {}

  route(request: LLMRequest, budget: TokenBudget): ModelRoute {
    const policy =
      this.routeConfigRepository.findByScene(request.scene) ??
      this.routeConfigRepository.defaultPolicy();

    for (const model of policy.candidates) {
      if (!budget.fits(model)) continue;
      if (!this.modelHealthService.isAvailable(model)) continue;
      return ModelRoute.of(model, policy.providerOf(model), policy);
    }

    throw new NoAvailableModelError(request.scene);
  }
}
\end{lstlisting}
...
interface LLMRateLimiter {
  acquire(tenantId: string, userId: string, model: string, budget: TokenBudget): Promise<RateLimitPermit>;
  reconcile(permit: RateLimitPermit, actualUsage: TokenUsage): Promise<void>;

...
\begin{itemize}[itemsep=2pt, topsep=4pt]
  \item 大模型 Token 是怎么计算的？上下文窗口越大越好吗？
  \item Function Calling 和 MCP 有什么区别？工具调用怎么做权限控制？
  \item RAG 召回率低怎么排查？Chunk 怎么切？Rerank 解决什么问题？
  \item Agent 的 Memory 怎么设计？长任务上下文溢出怎么办？
  \item 如何设计一个生产级 AI 应用？模型网关、评测、可观测怎么做？
\end{itemize}


这些题不是背几个术语就能过的。AI 应用开发面试更看重的是：\textbf{你能不能把大模型、RAG、Agent、工具调用和系统设计放到真实工程里理解。}
...
\begin{itemize}[itemsep=2pt, topsep=4pt]
  \item 这篇文章是入口，帮你快速定位高频考点。
  \item AI 应用开发专栏 是正文，帮你把每个考点背后的原理、工程细节和实践方案讲透。
\end{itemize}

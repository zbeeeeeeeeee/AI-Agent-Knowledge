...
\section{1. 请用一句话概括 AI Agent 是什么？它和普通 Chatbot 有什么区别？ ⭐⭐}
...
\section{2. Agent Loop 的完整流程是什么？工程难点在哪里？ ⭐⭐}
...
\section{3. ReAct、Plan-and-Execute、Reflection、Multi-Agent 这些范式分别适合什么场景？ ⭐⭐⭐}
...
\section{4. Tools 注册时，工具 description 为什么非常关键？ ⭐⭐}
...
\section{5. Agent 的短期记忆和长期记忆有什么区别？分别怎么落地？ ⭐⭐}
...
\section{6. 记忆系统需要解决哪些核心问题？如何避免记忆污染？ ⭐⭐⭐}
...
\section{7. 向量记忆和 Markdown 记忆分别适合什么场景？ ⭐⭐}
...
\section{8. 为什么 Agent 场景下只优化 Prompt 不够？Context Engineering 解决什么问题？ ⭐⭐⭐}
...
\section{9. MCP 协议解决什么问题？它和 Function Calling 有什么区别？ ⭐⭐}
...
\section{10. Agent Skills 是什么？Skill 路由怎么做？ ⭐⭐⭐}
...
\section{11. Multi-Agent 协作的主要问题是什么？为什么生产里不能盲目上多 Agent？ ⭐⭐⭐}
...
\section{12. 上下文利用率为什么有一个 40\% 的阈值现象？怎么利用这个现象做工程优化？ ⭐⭐⭐}
...
\section{13. 什么是 RAG？它和传统搜索引擎、微调有什么区别？ ⭐⭐}
...
\section{14. RAG 系统中 Chunk Size 怎么选？太大和太小分别有什么影响？ ⭐⭐}
...
\section{15. 余弦相似度、内积、欧氏距离分别适合什么 Embedding 模型？ ⭐⭐}
...
\section{16. 向量索引 HNSW、IVF、DiskANN 分别适合什么场景？ ⭐⭐⭐}
...
\section{17. RAG 评测为什么必须分检索和生成两段？怎么打分？ ⭐⭐⭐}
...
\section{18. 长上下文窗口会不会取代 RAG？什么时候用 RAG，什么时候直接用长上下文？ ⭐⭐⭐}
...
\section{19. 多模态 RAG 相比纯文本 RAG 多了哪些挑战？ ⭐⭐⭐}
...
\section{20. RAG 和 Agent 的记忆系统在技术上有哪些异同？ ⭐⭐⭐}
...
\section{21. Harness Engineering 是什么？为什么说 Agent = Model + Harness？ ⭐⭐}
...
\section{22. Harness 的六层架构分别解决什么问题？ ⭐⭐⭐}
...
\section{23. 为什么同一个模型换一套工具接口，效果能差 10 倍？ ⭐⭐⭐}
...
\section{24. Loop Engineering 是什么？它和 Agent Loop、Workflow Graph 有什么区别？ ⭐⭐⭐}
...
\section{25. Workflow、Graph、Loop 三者是什么关系？什么场景用哪种编排方式？ ⭐⭐⭐}
...
\section{26. 如何防止 Agent Loop 陷入死循环？ ⭐⭐}
...
\section{28. 模型 Fallback 策略怎么设计？什么时候不能自动降级？ ⭐⭐⭐}
...
\section{29. Token 成本怎么归因到租户、用户、功能和 Prompt 版本？ ⭐⭐⭐}
...
\section{30. Agent 的 AGENTS.md 应该怎么写？为什么不要写成超级 System Prompt？ ⭐⭐}
...
\section{31. 工作流中断后怎么恢复？State 更新策略怎么选？ ⭐⭐⭐}
...
\section{33. 请设计一个生产级 AI 应用的整体架构，从入口到模型返回的完整链路是什么？ ⭐⭐⭐}
...
\section{34. 同步、流式（SSE）、异步三种模式怎么选？ ⭐⭐}
...
\section{35. 为什么大模型调用限流要同时看 RPM、TPM、并发数和租户预算？ ⭐⭐⭐}
...
\section{36. AI 应用如何做超时、重试、限流、熔断和降级？ ⭐⭐⭐}
...
\section{37. 从 Prompt Demo 到生产级架构，最大的差距在哪？要补齐哪些能力？ ⭐⭐⭐}
...
\section{38. Prompt 注入攻击在系统设计层面怎么防？ ⭐⭐⭐}
...
\section{39. 如何设计一个实时语音 Agent 系统？ ⭐⭐⭐}
...
\section{40. 出现模型输出事故后，如何通过 Trace 回放定位问题？ ⭐⭐⭐}
...
\section{41. 如何构建 Golden Set？冷启动阶段没有生产日志怎么办？ ⭐⭐⭐}
...
\section{42. LLM-as-Judge 有哪些主要偏差？怎么校准？ ⭐⭐⭐}
...
\section{43. Agent 评测为什么比普通 RAG 和问答评测复杂得多？ ⭐⭐⭐}
...
\section{44. RAG、Agent、结构化输出的评测指标为什么不能混用一套？ ⭐⭐}
...
\section{45. Token 是什么？为什么中文、英文、代码消耗的 Token 不一样？ ⭐⭐}
...
\section{46. Temperature、Top-P、Top-K 分别控制什么？生产环境怎么设置更稳？ ⭐⭐}
...
\section{47. Streaming 为什么能改善用户体验？它能减少总耗时和 Token 成本吗？ ⭐⭐}
...
\section{48. JSON Mode 和 Structured Outputs 有什么区别？ ⭐⭐⭐}
...
\section{49. Function Calling 的完整链路是什么？和 MCP Agent Skill 是什么关系？ ⭐⭐⭐}
...
\section{50. 工具调用为什么必须做安全治理？安全边界在哪里？ ⭐⭐⭐}
...
\section{51. 哪些大模型 API 错误可以重试？哪些不能重试？为什么必须做幂等？ ⭐⭐}
...
\section{52. 大模型为什么会产生幻觉？常见缓解方案有哪些？ ⭐⭐}
...
\section{53. Token 预算怎么估算？输入、输出、历史消息、RAG 证据如何取舍？ ⭐⭐⭐}
...
\section{54. AI 应用调用日志里至少要记录哪些字段？为什么？ ⭐⭐}
...
\section{55. 如果 LLM-as-Judge 和人工评测结果不一致，应该怎么处理？ ⭐⭐⭐}

...
\title{AI Agent 记忆系统：短期记忆、长期记忆与记忆演化机制}
\date{}
\maketitle
\markboth{Java 版 Agent 知识}{ Java 版 Agent 知识}

\section*{Agent 的记忆系统是如何设计的？}


\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/mao/demos/latex_demos/ai-agent-knowledge/HTML/AI Agent 记忆系统：短期记忆、长期记忆与记忆演化机制 _ JavaGuide_files/agent-memory-memory-taxonomy.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/mao/demos/latex_demos/ai-agent-knowledge/HTML/AI Agent 记忆系统：短期记忆、长期记忆与记忆演化机制 _ JavaGuide_files/agent-memory-arch.jpg}
\caption*{AI Agent 记忆系统架构}
\end{figure}



\subsection*{记忆有哪些存储形式？}
...
{\footnotesize
\begin{tabular}{|p{\dimexpr 0.164\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.364\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.473\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{存储形式} & \textbf{说明} & \textbf{典型实现} \\
\hline
...
{\footnotesize
\begin{tabular}{|p{\dimexpr 0.105\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.237\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.368\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.289\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{功能类型} & \textbf{核心问题} & \textbf{存储内容} & \textbf{典型场景} \\
\hline
...
\begin{itemize}[itemsep=1pt, topsep=2pt]
  \item 情景记忆（Episodic Memory）：记录特定时间、场景下的具体事件，回答 “What happened?”。例如：“上周三用户反馈订单超时问题”。
  \item 语义记忆（Semantic Memory）：从多个情景中提炼出的通用知识、事实或规律，回答 “What does it mean?”。例如：“该用户对性能问题的敏感度高于功能需求”。
  \item 程序记忆（Procedural Memory）：存储技能、规则和习得行为，让 Agent 能自动执行某类任务序列，而不是每次重新推理。例如：“处理该用户的代码审查时，优先检查 OOM 风险”。
\end{itemize}


\subsection*{记忆操作的生命周期是怎样的？}


\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/mao/demos/latex_demos/ai-agent-knowledge/HTML/AI Agent 记忆系统：短期记忆、长期记忆与记忆演化机制 _ JavaGuide_files/agent-memory-lifestyle.jpg}
\caption*{记忆操作的生命周期}
\end{figure}
...
{\footnotesize
\begin{tabular}{|p{\dimexpr 0.049\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.415\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.537\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{操作} & \textbf{说明} & \textbf{工程实现} \\
\hline
...
\subsection*{什么是短期记忆（Short-Term Memory / Working Memory）？}


短期记忆是 Agent 在当前单次会话中持有的暂存信息，包括用户提问、模型每轮回复、工具调用的中间结果（Observations）。这些内容会直接进入当轮 Prompt，是当前任务状态的主要载体。宿主机侧的隐藏状态、\texttt{state} JSON 如果存在，也应该和这条叙事对齐。
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/mao/demos/latex_demos/ai-agent-knowledge/HTML/AI Agent 记忆系统：短期记忆、长期记忆与记忆演化机制 _ JavaGuide_files/context-utilization-40-percent-threshold-phenomenon.pdf}
...
\subsection*{什么是长期记忆（Long-Term Memory）？}
...
\subsection*{长期记忆和 RAG 有什么区别？}


\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/mao/demos/latex_demos/ai-agent-knowledge/HTML/AI Agent 记忆系统：短期记忆、长期记忆与记忆演化机制 _ JavaGuide_files/agent-memory-rag-vs-memory.pdf}
...
VectorStore 负责向量存储。它把提取出来的记忆文本转成 Embeddings，再存进向量数据库。以单节点 Qdrant 1.x 版本、本地 SSD、HNSW 索引 ef=128、Recall@10 ≥ 0.95 为基准，在低并发场景（如 QPS 小于 50）下，P99 延迟可以控制在数十毫秒级。不同产品在同样 QPS 下 P99 差异可能达到 5-10 倍，比如 Pinecone Serverless、自建 Qdrant、Milvus 之间就会有明显差异。实际选型最好参考 \href{https://ann-benchmarks.com/}{ann-benchmarks.com} 或各厂商 benchmark 报告。常见方案包括 Pinecone、Weaviate、Chroma、Qdrant 等。
...
{\footnotesize
\begin{tabular}{|p{\dimexpr 0.095\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.460\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.444\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{维度} & \textbf{关键考量} & \textbf{说明} \\
\hline
...
\subsection*{主流 Memory 产品如何对比？}


下面这张表只展示公开项目或产品各自的侧重点，不等于直接选型结论；最终取决于延迟要求、合规要求和数据形态。


{\footnotesize
\begin{tabular}{|p{\dimexpr 0.245\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.155\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.548\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.052\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{产品} & \textbf{核心思想} & \textbf{技术亮点} & \textbf{适用场景} \\
\hline
\href{https://github.com/mem0ai/mem0}{Mem0} & 单次 ADD-only 抽取 + 多信号融合检索 & 单次 LLM 调用完成实体抽取与跨记忆链接；语义 + BM25 + Entity Linking 并行打分；通过可选的 GraphStore 后端启用图记忆（Mem0g） & 通用对话记忆 \\
\hline
...
\subsection*{LETTA、ZEP、MemOS 有什么不同？}


LETTA 把上下文想成操作系统里的页。Main Context 放系统指令和当前工作台，FIFO 顶住最新消息；顶不住时，就把旧段落递归摘要后换到 External Context。这个思路很好理解，但它是一条有损路径。递归摘要多轮以后，精确密钥字面量、报错栈、小数点后几位这种细节很容易先被洗掉。看起来像“失忆”，是压缩带来的副作用。
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/mao/demos/latex_demos/ai-agent-knowledge/HTML/AI Agent 记忆系统：短期记忆、长期记忆与记忆演化机制 _ JavaGuide_files/agent-memory-evolution.jpg}
\caption*{记忆系统的高级演化机制}
\end{figure}



\subsection*{记忆反思与合成如何实现？}
...
其中 \texttt{decay(t)} 通常取指数形式，比如 \texttt{e\textasciicircum{}\{-λt\}}。这套机制来自《Generative Agents》提出的三维检索模型。实际工程里，不建议每次在向量库里对全量记忆计算时间衰减，更稳的做法是向量库先做静态语义召回，再在 Reranker 阶段实时应用动态调整。
...
很多团队低估记忆淘汰的必要性，倾向保留所有信息。结果向量库堆入几十万条记忆，每次 Top-K 混入过时噪音，Agent 的建议停留在三年前。该问题很难靠调 Prompt 修复。



\section*{如何优化长期记忆的检索效果？}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/mao/demos/latex_demos/ai-agent-knowledge/HTML/AI Agent 记忆系统：短期记忆、长期记忆与记忆演化机制 _ JavaGuide_files/agent-memory-retrieval-optimization.jpg}
\caption*{长期记忆的检索优化策略}
\end{figure}



\subsection*{混合检索与元数据过滤怎么做？}
...
\begin{itemize}[itemsep=1pt, topsep=2pt]
  \item RRF（Reciprocal Rank Fusion）：几乎不用调参，适合冷启动，按排名倒数加权融合。
  \item Linear weighted（\texttt{α·dense + (1-α)·sparse}）：可调，但需要标注数据校准权重。
  \item Cross-encoder Reranker：召回阶段取并集，精排阶段统一打分，对长尾 query 更有帮助。
\end{itemize}
...
Mem0 在 LoCoMo 上达到 91.6，较旧算法 +20 分；LongMemEval 上达到 93.4，+26 分；BEAM (1M) 上达到 64.1；每次检索约消耗 7K Token，对比全上下文方案的 25K+ 更省。详见 \href{https://docs.mem0.ai/core-concepts/memory-evaluation}{Mem0 官方 benchmark}。



“记忆没用”通常不是写入阶段完全失败，而是 Recall 跑偏，或精排未把真正相关内容顶上来。优先检查 trace 中的 query、过滤条件和融合权重，再决定是否给提取链路增加预算。不要急于增加写入逻辑，那可能只是把噪声写得更快。



\section*{生产级记忆系统架构要关注哪些要点？}
...
{\footnotesize
\begin{tabular}{|p{\dimexpr 0.093\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.186\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.721\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{维度} & \textbf{核心问题} & \textbf{解决方案} \\
\hline
...
\section*{如何用 Markdown 存储 Agent 记忆？}
...
\subsection*{为什么 Markdown 可以作为 Agent 记忆？}


Markdown 可以看成人机共写的明文长期记忆。不强制上向量检索，只靠目录组织，以及 Claude Code 里的 \texttt{@} / \texttt{rules} 机制，也能跑起来。
...
\begin{itemize}[itemsep=1pt, topsep=2pt]
  \item 透明可审计：随时打开文件，就能看到 Agent 记住了什么、写入了什么，没有黑盒。
  \item 持久化：文件存在磁盘上，不依赖进程生命周期。进程崩溃、重启、换机器，记忆都在。
  \item 版本控制：记忆可以提交到 Git，回滚、分支、Code Review 都很自然。
  \item 零迁移成本：标准格式，没有供应商锁定。换模型、换框架时，复制文件即可。
  \item 成本低：托管向量数据库和完整 RAG pipeline 的成本、运维复杂度都不低，Markdown 本地文件几乎没有额外成本。
\end{itemize}


Manus 把文件系统视为结构化外部记忆；Claude Code 把 \texttt{CLAUDE.md} 和 Auto Memory 产品化；OpenClaw 等 Agent 项目和社区实践中，也能看到类似的文件化记忆思路。它们都说明，在不少 Agent 场景里，文件系统 + Markdown 已经是足够务实的长期记忆方案。



\subsection*{Claude Code 的 `CLAUDE.md` 机制是怎样的？}


Claude Code 的记忆系统采用双轨制：人工编写的 \texttt{CLAUDE.md}，以及自动积累的 Auto Memory。



\subsubsection{`CLAUDE.md` 里该写什么、不该写什么？}


官方建议每个 \texttt{CLAUDE.md} 控制在 200 行以内。超过这个限制会降低 Claude 的指令遵守率。通过 \texttt{@} 引用拆分文件可以改善可维护性，但不会减少上下文消耗，因为被引用文件在启动时会全量加载。如果指令很长，优先使用 \texttt{.claude/rules/} 目录的 path-scoped rules，只在编辑匹配路径时加载对应规则。



可以把 \texttt{CLAUDE.md} 理解成给 AI 新人的 onboarding 文档。写得不好还不如不写，因为臃肿的 \texttt{CLAUDE.md} 会把真正重要的规则淹掉。



适合写进去的内容有几类。技术栈和版本信息很重要，框架版本差异往往是 AI 犯错的源头。不标注 Spring Boot 版本时，模型容易生成训练数据中更常见的版本用法。常用命令也应该写进去，比如构建、测试、lint、启动，并尽量放在代码块里。代码块里的命令 Claude 更倾向于照着跑，自然语言里的命令它可能会按自己的理解改写。
...
一个判断标准很好用：逐行看 \texttt{CLAUDE.md}，每条都问自己，如果没有这行，Claude 最近是否真的犯过这个错。如果答案是“好像没有”，那它大概率可以删。



\subsubsection{怎么写才能让 Claude 真正遵守？}
...
标记词可以用，但别滥用。如果某条规则 Claude 反复违反，加 \texttt{IMPORTANT:} 或 \texttt{YOU MUST:} 能稍微提高注意力。但整篇文件到处都是“重要”，最后就等于没有重点。
...
\subsubsection{`CLAUDE.md` 文件的层级结构是怎样的？}

{\footnotesize
\begin{tabular}{|p{\dimexpr 0.032\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.389\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.063\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.516\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{层级} & \textbf{位置} & \textbf{作用范围} & \textbf{适用场景} \\
\hline
组织级 & 系统目录，如 \texttt{/etc/claude-code/CLAUDE.md} & 所有用户 & 公司编码规范、安全策略，任何设置都无法排除 \\
\hline
用户级 & \texttt{\textasciitilde{}/.claude/CLAUDE.md} & 个人所有项目 & 代码风格偏好、个人工具习惯 \\
\hline
项目级 & \texttt{./CLAUDE.md} 或 \texttt{./.claude/CLAUDE.md} & 团队共享 & 项目架构、编码标准、工作流，提交至 Git \\
\hline
本地级 & \texttt{./CLAUDE.local.md} & 个人当前项目 & 沙箱 URL、测试数据偏好，需手动加入 \texttt{.gitignore}，运行 \texttt{/init} 可自动添加 \\
\hline
\end{tabular}
}
...
文件加载遵循目录树向上查找规则：从当前工作目录逐级向上。同一目录内，\texttt{CLAUDE.local.md} 会追加在 \texttt{CLAUDE.md} 之后，越靠近工作目录的规则优先级越高。



\texttt{CLAUDE.md} 不适合存大段日志和完整对话记录，也不应该存敏感密钥、Token、账号信息。高频变化的运行时数据、可以实时查询的动态信息，也不适合写进去。



项目变大后，需要做分层管理。一个人的项目，一份 \texttt{CLAUDE.md} 通常够用；团队项目就要拆开。
...
可以用 \texttt{@path/to/file} 引用外部文件。但要注意，\texttt{@} 引用最多支持 5 层递归深度。首次在项目中使用外部引用时，Claude Code 会弹出审批对话框。如果误拒，引用会被永久禁用，需要手动重置。\texttt{@} 引用会把整个文件内容嵌入上下文，被引用文件在启动时全量加载，所以不会减少上下文消耗。



如果需要更细粒度控制，可以用 \texttt{.claude/rules/} 目录组织 path-scoped rules。它和 \texttt{@} 引用的区别很关键：rules 只在匹配指定路径时加载，属于按需加载；\texttt{@} 引用在启动时全量加载。规则只针对特定文件或目录时，比如后端 API 规范、测试配置，优先用 rules，而不是继续往 \texttt{CLAUDE.md} 里堆内容。
...
\subsubsection{[AGENTS.md](http://agents.md/) 和 [CLAUDE.md](http://claude.md/) 是什么关系？}


Claude Code 读取 \texttt{CLAUDE.md}，不是 \texttt{AGENTS.md}。\texttt{AGENTS.md} 更像跨工具开放标准，被 OpenAI Codex、Cursor 等采用。如果仓库已经用 \texttt{AGENTS.md} 给其他编码 Agent 提供指令，可以创建一个导入 \texttt{AGENTS.md} 的 \texttt{CLAUDE.md}，让两个工具复用同一份基础指令，不用重复维护。
...
\subsubsection{Auto Memory 是什么？}


Auto Memory 是 Claude 根据对话自动写入的笔记，包括调试模式、代码习惯、工作流偏好。它存在 \texttt{\textasciitilde{}/.claude/projects/<project>/memory/} 目录下，\texttt{MEMORY.md} 是入口文件，细节笔记放在子文件中。



这里有几个使用限制要记住。\texttt{MEMORY.md} 只加载前 200 行或 25KB，超出部分不会被读取，Claude 会把详细内容拆分到 Topic 文件里。经过 20-30 个会话后，Auto Memory 笔记质量可能下降，出现矛盾条目或过时信息累积。社区里有 dream-skill 这类工具能做记忆整合，比如 Orient、Gather Signal、Consolidate、Prune 四阶段，但这不是官方正式功能。



如果要禁用 Auto Memory，除了 \texttt{/memory} 切换和 \texttt{autoMemoryEnabled} 配置，也可以通过环境变量 \texttt{CLAUDE\_CODE\_DISABLE\_AUTO\_MEMORY=1} 禁用。CI/CD 场景更适合用这种方式，因为自动化管线没必要让 Claude 积累构建环境笔记。
...
\begin{itemize}[itemsep=1pt, topsep=2pt]
  \item 用户级记忆：存个人偏好和长期习惯，放在 \texttt{\textasciitilde{}/.claude/CLAUDE.md}，比如 2-space 缩进、先写测试再写代码、不喜欢用 emoji。
  \item 项目级记忆：存项目规范、技术栈、目录结构，放在仓库根目录的 \texttt{CLAUDE.md}，团队成员共享，通过 Git 同步。
  \item 子目录级记忆：存局部模块的专属规则，放在子目录的 \texttt{CLAUDE.md}，比如 \texttt{backend/} 下的 API 设计规范、\texttt{docs/} 下的写作风格要求。
  \item 团队共享记忆：需要提交到仓库的共同约定，通常是项目级 \texttt{CLAUDE.md} 和 \texttt{.claude/rules/} 目录下可版本化的规则文件。
  \item 私有记忆：不应该提交的个人工作流，比如 \texttt{CLAUDE.local.md}，加入 \texttt{.gitignore} 后只留在本地。
\end{itemize}


\subsection*{Markdown 记忆和传统长期记忆的边界在哪里？}


\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/mao/demos/latex_demos/ai-agent-knowledge/HTML/AI Agent 记忆系统：短期记忆、长期记忆与记忆演化机制 _ JavaGuide_files/agent-memory-markdown-memory-boundary.pdf}
\caption*{Markdown 记忆和传统长期记忆的适用边界}
\end{figure}
...
{\footnotesize
\begin{tabular}{|p{\dimexpr 0.079\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.286\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.159\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.254\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.222\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{维度} & \textbf{Markdown 记忆} & \textbf{向量库记忆} & \textbf{RAG 知识库} & \textbf{数据库型框架（Mem0 等）} \\
\hline
...
Markdown 的局限也很明显：需要从海量非结构化文本中检索特定片段时，人工组织的 Markdown 会成为瓶颈，此时向量库的语义检索能力不可替代。
...
\subsection*{Markdown 记忆应如何维护？}


以 \texttt{CLAUDE.md} 为例。\texttt{CLAUDE.md} 不是写完就完事，项目会演进，规则也会过时。
...
规则最好错误驱动地持续进化。每次纠正 Claude 的错误后，可以追加一句“更新 \texttt{CLAUDE.md}，确保下次不再犯”。累积几次同类错误后，再归纳成一条精炼规则，避免文件快速膨胀。



两个预警信号。第一，Claude 为已经写在文件里的规则道歉，比如“抱歉，我刚才忽略了 XX 规则”。这说明规则表述可能不够直接。第二，同一条规则在不同会话中反复被违反。这通常不是措辞问题，而是整份文件太长，规则被稀释了。解决方式不是继续改措辞，而是压缩整份文件。



维护时做对话式审查：每隔几周挑几条 \texttt{CLAUDE.md} 规则询问 Claude“删除这条规则是否影响行为”；若无影响，可删除。
...
\texttt{/init} 也可以用，但不要直接用。自动生成的 \texttt{CLAUDE.md} 是一个不错的起点，但里面可能有不准确的项目描述。按上面的原则逐条审查，删掉冗余，补上遗漏。
...
\texttt{CLAUDE.md} 不是写得越多越好。每一条规则都应该对应 Claude 真实犯过的错误。如果删掉某条之后 Claude 行为没变，那它可能从来就没起作用。

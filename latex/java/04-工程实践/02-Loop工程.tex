...
代码 Agent 真能连续读文件、改代码、跑命令、处理 PR 之后，我们确实不能只盯着“下一句 Prompt 怎么写”。以前是人守在对话框前，一轮一轮补充提示；现在越来越多任务会由 \texttt{/loop}、\texttt{/goal}、CI、PR 评论或者定时任务触发。Agent 被叫醒后自己读材料、跑命令、写状态，卡住再把问题抛回来。
...
从公开讨论看，这个词大概在 \textbf{2026 年 6 月上旬} 开始热起来。Addy Osmani 在 2026 年 6 月 7 日写了篇 Loop Engineering，随后 AI 圈开始反复讨论“让系统去提示 Agent”。



我个人的感觉是：新鲜的是名字，能力早就在往这个方向走了。Claude Code、Codex 里的 \texttt{/loop}、\texttt{/goal}、Automations、Skills、Sub-agent、工作区隔离、MCP/Connector，解决的都是一类问题：\textbf{别让 Agent 只停在一轮回答里，给它边界，让它继续干活。}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/zhang/demos/latex_demos/Agent知识/HTML/Loop Engineering 是什么？为什么说它是新瓶装旧酒？ _ JavaGuide_files/loop-engineering-outer-loop.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/zhang/demos/latex_demos/Agent知识/HTML/Loop Engineering 是什么？为什么说它是新瓶装旧酒？ _ JavaGuide_files/agent-loop-flow.jpg}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/zhang/demos/latex_demos/Agent知识/HTML/Loop Engineering 是什么？为什么说它是新瓶装旧酒？ _ JavaGuide_files/workflow-graph-loop-relation.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/zhang/demos/latex_demos/Agent知识/HTML/Loop Engineering 是什么？为什么说它是新瓶装旧酒？ _ JavaGuide_files/context-engineering-vs-context-engineering-dimension-comparison.jpg}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/zhang/demos/latex_demos/Agent知识/HTML/Loop Engineering 是什么？为什么说它是新瓶装旧酒？ _ JavaGuide_files/harness-engineering-layers-arch.jpg}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/zhang/demos/latex_demos/Agent知识/HTML/Loop Engineering 是什么？为什么说它是新瓶装旧酒？ _ JavaGuide_files/skills-agent-execution-link.jpg}
\caption*{Agent 执行链路}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/zhang/demos/latex_demos/Agent知识/HTML/Loop Engineering 是什么？为什么说它是新瓶装旧酒？ _ JavaGuide_files/agent-skills-progressive-disclosure.png}
\caption*{Skill 渐进式披露}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/zhang/demos/latex_demos/Agent知识/HTML/Loop Engineering 是什么？为什么说它是新瓶装旧酒？ _ JavaGuide_files/mcp-simple-diagram.jpg}
\caption*{MCP 图解}
\end{figure}
...
我在 Claude Code 核心命令详解 里专门写过 \texttt{/loop}。这里顺手把 \texttt{/loop} 和 \texttt{/goal} 分开说一下，这俩都和 Loop Engineering 有关，但用法差得挺远。



\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/zhang/demos/latex_demos/Agent知识/HTML/Loop Engineering 是什么？为什么说它是新瓶装旧酒？ _ JavaGuide_files/claudecode-father-loop.jpg}
\caption*{Claude Code 推荐使用 loop 命令}
\end{figure}



\texttt{/loop} 主要解决“过一会儿再看一次”的问题。它会在当前 session 里重复运行一个 prompt。你可以给固定间隔，比如每 5 分钟检查一次部署；也可以不给间隔，让 Claude 根据观察结果自己选择下一次等待多久。
...
\texttt{/goal} 主要解决“这个目标有没有完成”的问题。你给它一个可验证完成条件，Claude 会一轮一轮推进；每轮结束后，由一个独立的小模型基于对话里已经出现的证据判断条件是否满足。不满足就继续，满足就停止。
...
我自己的记法是：\texttt{/loop} 管下一次什么时候醒，\texttt{/goal} 管什么时候算做完。
...
我的使用习惯是：跑 \texttt{/loop} 前先收紧权限，写清楚轮询目标和停止条件；跑 \texttt{/goal} 前把完成条件写成可验证结果，并要求 Claude 展示测试、构建或 diff 检查结果。
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/zhang/demos/latex_demos/Agent知识/HTML/Loop Engineering 是什么？为什么说它是新瓶装旧酒？ _ JavaGuide_files/loop-engineering-ci-triage-loop.png}
...
\begin{itemize}[itemsep=1pt, topsep=2pt]
  ...
  \item 验证信号很弱，只能靠 Agent 自己说“我觉得可以了”。
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/zhang/demos/latex_demos/Agent知识/HTML/Loop Engineering 是什么？为什么说它是新瓶装旧酒？ _ JavaGuide_files/loop-engineering-safety-boundary.png}
\caption*{Loop 的安全边界}
\end{figure}

...
\title{AI Agent 核心概念：Agent Loop、Plan-and-Execute、A2A、Agentic Workflows、Tools 注册}
\date{}
\maketitle
\markboth{Python 版 Agent 知识}{ Python 版 Agent 知识}

\section*{AI Agent 的演进}
...
\textbf{2022 年，ChatGPT 这类产品刚火时}，主要应用仍是与模型“对话”。模型能基于已有知识回答问题，不能主动调用外部工具，也不能自己完成操作。
...
\textbf{2023 年中，Function Calling 出现后，事情开始变了。}
...
\textbf{2025 年，Agent 开始往长任务执行方向走。}
...
\textbf{到了 2026 年，Agent 开始更接近长期在线的数字工作单元。}
...
\subsection*{Agent、传统编程和 Workflow 区别？}
...
\begin{itemize}[itemsep=1pt, topsep=2pt]
  \item 长任务跑久了，历史信息会被截断，模型会”失忆”。更烦的是，上下文变长后推理质量不一定更好，很多模型对中间位置的信息利用效率并不高
  \item 工具调用可以降低幻觉，但不能彻底消灭。LLM 在推理步骤里仍然可能生成错误判断，工具返回结果也不一定能把它拉回来
  \item 多轮迭代、工具调用、日志回传、上下文压缩，每一项都在烧 Token。复杂任务跑一轮，账单可能真会让人清醒
  \item Agent 能执行代码、调 API、读写文件，也就一定会面对 Prompt Injection 和越权操作风险。更现实的做法是权限最小化、沙箱隔离、高危操作人工确认
  \item 深度多步推理任务里，LLM 还是容易局部最优，可能看起来一直在推进，已经偏题了
  \item Agent 为什么做了某个决策、为什么调用了某个工具、是哪一步把上下文带偏了，排查起来很头疼
\end{itemize}
...
一般可以用这个公式概括：\textbf{Agent = LLM + Planning + Memory + Tools} 。



\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/mao/demos/latex_demos/ai-agent-knowledge/HTML/AI Agent 核心概念：Agent Loop、Plan-and-Execute、A2A、Agentic Workflows、Tools 注册 _ JavaGuide_files/agent-core-arch.jpg}
\caption*{AI Agent 核心架构}
\end{figure}



\textbf{推理与规划（Reasoning / Planning）}：用 LLM 分析当前任务状态，拆目标，决定下一步怎么做。Chain-of-Thought（CoT）提示技术可以让模型逐步推理，减少直接拍脑袋给答案的概率。
...
\textbf{Tools（工具）}：让 LLM 能真正操作外部世界，比如查数据、调 API、读文件、执行代码。没有工具，Agent 大多只能停留在“给出建议”。
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/mao/demos/latex_demos/ai-agent-knowledge/HTML/AI Agent 核心概念：Agent Loop、Plan-and-Execute、A2A、Agentic Workflows、Tools 注册 _ JavaGuide_files/agent-loop-flow.jpg}
...
\begin{enumerate}[itemsep=1pt, topsep=2pt]
  \item 初始化时加载 System Prompt、可用工具列表、用户初始请求
  \item 循环迭代——读取上下文，LLM 推理决定下一步（调用工具还是直接回复），触发并执行工具，捕获返回结果追加到上下文
  \item LLM 判断任务完成，不再调用工具时退出循环
  \item 安全兜底——防止死循环，设置最大迭代轮次上限（一般 10 到 20 轮）或 Token 消耗阈值
\end{enumerate}
...
\subsection*{做一个 Agent 系统，最少要搞定哪三层？}
...
\begin{enumerate}[itemsep=1pt, topsep=2pt]
  \item \textbf{LLM Call} ：这一层负责模型调用。比如 OpenAI、Anthropic、Hugging Face 的接口差异，流式输出，Token 截断，重试机制，都在这里处理。
  \item \textbf{Tools Call} ：这一层负责让 LLM 和外部系统交互。Function Calling、MCP、Skills 都可以放在这里看。读写本地文件、网页搜索、代码沙箱、第三方 API 调用，都属于工具能力。
  \item \textbf{Context Engineering} ：这一层负责管理传给大模型的 Prompt 和上下文。狭义看，它是系统提示词编排。放宽一点，它还包括动态记忆注入、会话状态管理、工具描述动态组装。
\end{enumerate}
...
\subsection*{数据格式：Function Calling Schema}
...
\textbf{1. 传统 Toolkits（黑盒）}：把多个原子工具在代码层封装成一个高阶工具，对外只暴露 JSON Schema，LLM 看不到内部执行路径。推理步骤少、Token 消耗低，适合逻辑固定的场景。



\textbf{2. Agent Skills（白盒）}：以 \texttt{SKILL.md} 为核心的自然语言指令集。每个 Skill 是一个独立文件夹：
...
\texttt{SKILL.md} 分两部分：前面是轻量元数据，告诉宿主”我是谁、什么时候该用我”；后面是正文，写具体流程、约束和示例。启动时只读元数据做发现，等 LLM 判断需要某个 Skill，再把完整正文加载进上下文。这种延迟加载设计，是 Agent Skills 和传统 Toolkits 最大的不同。



Claude Code、Cursor 这类工具已经原生支持这套模式，会自动扫描项目里的 \texttt{.claude/skills/} 目录，由模型自己判断哪个 Skill 该激活。



纯代码封装、调用路径固定，用 Toolkits。团队经验沉淀、任务流程灵活，用 Agent Skills 更合适。更详细的 Skills 工程实践——包括路由设计、\href{http://skill.md/}{SKILL.md} 写法避坑、第三方 Skill 安全审计，可以看：《Agent Skills 详解》。



\subsection*{通信接入：MCP 协议}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/mao/demos/latex_demos/ai-agent-knowledge/HTML/AI Agent 核心概念：Agent Loop、Plan-and-Execute、A2A、Agentic Workflows、Tools 注册 _ JavaGuide_files/mcp-simple-diagram.jpg}
\caption*{MCP 图解}
\end{figure}
...
{\footnotesize
\begin{tabular}{|p{\dimexpr 0.231\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.385\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.385\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{原语类型} & \textbf{作用} & \textbf{例子} \\
\hline
...
\section*{什么是 Prompt Engineering？}
...
\section*{什么是 Context Engineering？}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/mao/demos/latex_demos/ai-agent-knowledge/HTML/AI Agent 核心概念：Agent Loop、Plan-and-Execute、A2A、Agentic Workflows、Tools 注册 _ JavaGuide_files/context-engineering-vs-context-engineering-dimension-comparison.jpg}
...
\section*{Agent 核心范式有哪些？}


\subsection*{ReAct}


ReAct 是 Reasoning + Acting，由 Shunyu Yao 等人在 2022 年提出，论文是\href{https://react-lm.github.io/}{《ReAct: Synergizing Reasoning and Acting in Language Models》}。
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/mao/demos/latex_demos/ai-agent-knowledge/HTML/AI Agent 核心概念：Agent Loop、Plan-and-Execute、A2A、Agentic Workflows、Tools 注册 _ JavaGuide_files/ReAct-LLM.jpg}
\caption*{ReAct-LLM}
\end{figure}
...
接着去查服务负责人，通讯录里找到王建国，邮箱是 \href{mailto:wangjianguo@company.com}{wangjianguo@company.com}。
...
\begin{enumerate}[itemsep=1pt, topsep=2pt]
  \item 历史上下文，保存推理步骤、执行动作、反馈观察
  \item 实时环境输入，比如系统告警、用户反馈等外部变量
  \item LLM 推理模块：负责逻辑分析和下一步规划
  \item 工具集与技能库，包括原子工具和 Skills
  \item 反馈观察机制，采集工具响应并追加回上下文
\end{enumerate}


\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/mao/demos/latex_demos/ai-agent-knowledge/HTML/AI Agent 核心概念：Agent Loop、Plan-and-Execute、A2A、Agentic Workflows、Tools 注册 _ JavaGuide_files/agent-react-flow.jpg}
\caption*{ReAct 模式流程}
\end{figure}
...
\begin{itemize}[itemsep=1pt, topsep=2pt]
  \item Reflexion 框架：任务失败后进行口头反思，把结论存进记忆缓冲区，下次再遇到类似问题时参考。比如代码调试失败后，模型反思出”变量 count 在调用前没初始化”，下一轮就能规避。
  \item Self-Refine 方法：任务完成后，让模型审查自己的输出，再迭代改进。它通常用来提升回答、代码、文案这类输出质量。
  \item CRITIC 方法：引入外部工具，比如搜索引擎或代码执行器，对输出做事实验证，再根据验证结果修正。
\end{itemize}
...
\begin{enumerate}[itemsep=1pt, topsep=2pt]
  \item \textbf{Orchestrator-Subagent 模式} ：这是现在比较主流的形式。编排 Agent 负责全局规划和任务分发，子 Agent 并行或串行执行具体任务，最后汇总输出。
  \item \textbf{Peer-to-Peer 模式}：Agent 之间平等对话，互相审查，适合需要辩论、评审、验证的任务。
\end{enumerate}


\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/mao/demos/latex_demos/ai-agent-knowledge/HTML/AI Agent 核心概念：Agent Loop、Plan-and-Execute、A2A、Agentic Workflows、Tools 注册 _ JavaGuide_files/agent-multi-agent-arch.jpg}
\caption*{Multi-Agent 系统架构}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/mao/demos/latex_demos/ai-agent-knowledge/HTML/AI Agent 核心概念：Agent Loop、Plan-and-Execute、A2A、Agentic Workflows、Tools 注册 _ JavaGuide_files/agent-a2a.jpg}
\caption*{A2A 协议架构}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/mao/demos/latex_demos/ai-agent-knowledge/HTML/AI Agent 核心概念：Agent Loop、Plan-and-Execute、A2A、Agentic Workflows、Tools 注册 _ JavaGuide_files/agent-agentic-workflows.jpg}
\caption*{智能体工作流核心模式}
\end{figure}
...
\begin{enumerate}[itemsep=1pt, topsep=2pt]
  \item Reflection——让模型检查自己的工作
  \item Tool Use——给 LLM 配网络搜索、代码执行等工具
  \item Planning——让模型提出多步计划并执行
  \item Multi-agent Collaboration——多个 Agent 协作完成任务
\end{enumerate}
...
\subsection*{工作流里的 Node、Edge、State 是什么？}
...
“审核不通过就回到修改，最多重试 3 次”，翻译成图结构，是一条从 ReviewNode 指向 ReviseNode 的条件边，加上 \texttt{iteration\_count >= 3} 时跳到 ExitNode 的安全边界。State 里的 \texttt{iteration\_count} 是让这条逻辑能跑起来的关键。
...
\subsection*{什么时候用 Agent，什么时候用 Workflow？}
...
{\footnotesize
\begin{tabular}{|p{\dimexpr 0.300\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.283\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.417\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{场景特征} & \textbf{推荐方向} & \textbf{代价} \\
\hline

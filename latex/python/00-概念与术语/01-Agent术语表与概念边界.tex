...
\begin{quote}
本文是 Agent 知识库的\textbf{统一术语入口}。目标不是罗列名词，而是把最容易被混用的概念边界讲清楚：Agent、Chatbot、Workflow、RAG、MCP、A2A、Skills、Harness 等。
\end{quote}
...
\begin{itemize}[itemsep=1pt, topsep=2pt]
  \item 有人把“带工具的 Chatbot”叫 Agent；
  \item 有人把“固定流程 + LLM 节点”叫 Agent；
  \item 有人把 MCP 当作 Function Calling 的替代品；
  \item 有人把 RAG 直接等同于长期记忆。
\end{itemize}


如果团队内部口径不统一，设计评审、故障复盘、面试考察都会各说各话。本文约定：\textbf{Agent 知识库内的术语，以本页定义为准；文档冲突时，以本页为准。}
...
\begin{enumerate}[itemsep=1pt, topsep=2pt]
  \item 写文档时优先使用本页定义；出现新术语先补充到本页再使用。
  \item 面试题、架构图、设计文档中的“Agent”不得同时混用 Chatbot 与 Workflow 含义。

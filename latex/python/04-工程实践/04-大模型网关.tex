...
\title{大模型网关详解：多模型路由、Fallback、限流与成本控制}
\date{}
\maketitle
...
\begin{enumerate}[itemsep=1pt, topsep=2pt]
  \item \textbf{LLM Gateway 到底是什么}：它和传统 API 网关、LLM Router、RAG、Agent、MCP 分别是什么关系。
  \item \textbf{为什么不能所有请求都用最强模型}：如何按任务类型、成本、延迟、风险做多模型路由。
  \item \textbf{生产级 LLM Gateway 需要哪些能力}：统一接入、Fallback、限流、Token 预算、成本归因、观测审计和缓存。
  \item \textbf{如果让你设计一个 LLM Gateway，应该怎么拆}：组件拆分、请求生命周期、路由演进路线和路由错误兜底。
  \item \textbf{主流方案怎么选}：自研、LiteLLM、Cloudflare AI Gateway、Kong AI Gateway、Inworld Router、LLMRouter 各自适合什么团队。
\end{enumerate}
...
\subsection*{LLM Gateway 到底是什么？}


LLM Gateway 更像是：\textbf{API 网关能力 + 模型调用控制面}。



传统 API 网关是位于客户端与后端服务之间的\textbf{统一入口}，所有客户端请求先经过网关，再由网关路由到具体的目标服务，主要管 HTTP 流量：鉴权、限流、转发、日志、熔断。



\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/zhang/demos/latex_demos/Agent知识/HTML/大模型网关详解：多模型路由、Fallback、限流与成本控制 _ JavaGuide_files/api-gateway-overview.jpg}
\caption*{传统 API 网关示意图}
\end{figure}
...
更准确地说，\textbf{LLM Gateway 是应用层和模型供应商之间的一层治理入口}。它不一定替代企业已有的 API 网关，但会把模型调用相关的路由、预算、审计和适配逻辑收口。



\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{/home/zhang/demos/latex_demos/Agent知识/HTML/大模型网关详解：多模型路由、Fallback、限流与成本控制 _ JavaGuide_files/llm-gateway-overview.jpg}
\caption*{LLM 网关示意图}
\end{figure}
...
{\footnotesize
\begin{tabular}{|p{\dimexpr 0.164\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.475\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.361\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{直连模型的典型问题} & \textbf{线上表现} & \textbf{Gateway 对应能力} \\
\hline
...
\subsection*{LLM Gateway 和 LLM Router 有什么区别？}
...
{\footnotesize
\begin{tabular}{|p{\dimexpr 0.073\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.327\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.600\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{维度} & \textbf{LLM Router} & \textbf{LLM Gateway} \\
\hline
...
可以这么理解：\textbf{Router 负责选模型，Gateway 负责把整次模型调用管起来}。
...
\subsection*{LLM Gateway 和 RAG、Agent、MCP 是什么关系？}
...
{\footnotesize
\begin{tabular}{|p{\dimexpr 0.058\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.314\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.628\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{概念} & \textbf{主要解决什么问题} & \textbf{和 Gateway 的关系} \\
\hline
...
所以，Gateway 更靠近“模型调用治理”；RAG、Agent、MCP 更靠近“应用能力组织”。一个复杂 Agent 可以在多个步骤里调用 Gateway，Gateway 也可以对每个步骤分别记录 \texttt{scene}、\texttt{route\_reason}、Token 使用量和成本。



\subsection*{LLM Gateway 会不会增加延迟？}
...
\begin{itemize}[itemsep=1pt, topsep=2pt]
  \item 简单任务直接丢给小模型，不用每次都排大模型的队。
  \item 重复问题走缓存，模型都不用调。
  \item 语音交互、在线客服这类延迟敏感的场景，优先选 TTFT 更稳定的模型或供应商。
  \item 供应商抖动时快速 Fallback，别让用户干等到超时。
  \item 上下文太长的请求提前压缩，省得模型端慢慢算。
\end{itemize}
...
\subsection*{你真的需要 LLM Gateway 吗？}
...
如果你只是做内部工具、单模型、低流量、没有多租户、没有严格成本压力，也不需要复杂审计，那就先别过度设计。一个封装良好的 \texttt{LLMClient}，加上基础日志、超时、重试和错误处理，已经够用了。
...
\begin{itemize}[itemsep=1pt, topsep=2pt]
  \item \textbf{只有一个应用、一个模型、每天几百次调用} → 先不用 Gateway，把精力花在业务逻辑上。
  \item \textbf{有多个业务线或团队都在调用模型} → 开始收口，统一入口和调用规范。
  \item \textbf{有多租户、配额管理、成本需要按团队或场景归因} → 需要 Gateway。
  \item \textbf{有多供应商、需要 Fallback、想做模型路由} → 需要 Gateway。
  \item \textbf{有合规审计、Prompt 版本管理、线上质量回放、敏感内容拦截} → 必须 Gateway 化。
\end{itemize}
...
\section*{为什么不能所有请求都用最强模型？}


\subsection*{最贵的模型不一定是最适合的模型}
...
\begin{enumerate}[itemsep=1pt, topsep=2pt]
  \item \textbf{成本不可控}：分类、改写、摘要这类任务也走旗舰模型，单次看不贵，流量上来后很吓人。
  \item \textbf{延迟不稳定}：强推理模型为了复杂任务设计，不一定适合实时对话、语音交互、轻量判断。
  \item \textbf{资源被浪费}：简单任务没有给强模型发挥空间，复杂任务反而可能因为上下文组织差而答不好。
\end{enumerate}


以一组常见的内部模型分层为例：\texttt{tier-fast} 更适合低成本、快响应场景，\texttt{tier-pro} 更适合复杂推理和高质量输出。具体价差不要写死在业务代码里，应该放在模型注册表或价格快照里维护。



Gateway 在这里解决的不只是钱，还有后续换模型、控延迟、查问题时的混乱：\textbf{它要根据质量、成本、延迟做取舍，而不是固定选一个最强模型}。



\subsection*{什么任务适合小模型？什么任务必须上强模型？}
...
\href{https://github.com/ulab-uiuc/LLMRouter}{LLMRouter} 这类智能路由项目，思路是为每个查询动态选择更合适的模型，从而在质量、成本和延迟之间做取舍。它覆盖了单轮路由、多轮路由、个性化路由、Agentic 路由等方向，也提供 KNN、SVM、MLP、Matrix Factorization、Elo Rating、Graph-based routing 等策略。



这些策略适合学习和实验，但生产里要先解决可解释性和回放能力。更稳的路线是：\textbf{模型路由从简单规则出发，然后根据实际场景慢慢演进成可训练、可评估、可迭代的系统}。
...
{\footnotesize
\begin{tabular}{|p{\dimexpr 0.164\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.358\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.299\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.179\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{**路由策略**} & \textbf{**怎么做**} & \textbf{**适合场景**} & \textbf{**风险**} \\
\hline
...
\textbf{固定规则路由是起步首选}。直接按任务、用户类型、请求信息指定对应模型，好落地、结果可控；代价是规则要人工维护，业务调整后容易滞后。做第一版网关时，这个取舍通常可以接受。
...
\textbf{成本优先 / 级联路由（Cascade）} 是规则路由之后常见的进阶方式：先让小模型处理，解析失败、置信度低、质量校验不通过时再升级到更强模型。
...
\textbf{语义 / 分类路由}把路由决策从硬编码规则升级到基于内容理解。常见做法是把 Query 编码成 embedding，再和任务原型、模型 profile 或参考 prompt 做相似度匹配；也可以用轻量分类器判断请求复杂度和风险等级。
...
\textbf{学习型路由}门槛最高，得靠充足的标注、质量、成本和延迟数据训练。没有评测集和线上反馈，模型选择器很难解释，出了问题也不好复盘。



在此基础上的\textbf{个性化路由}，按用户习惯适配模型，适合 C 端，但隐私和调试难度大。



\textbf{Agentic 路由}是更复杂的一类方向。在多轮 Agent 任务中，路由不再是一次性决策——Agent 在执行过程中可能需要在不同步骤调用不同模型（比如规划步骤用强模型、工具调用用快模型、总结步骤用便宜模型），甚至需要根据中间结果动态调整后续步骤的模型选择。



\section*{LLM Gateway 需要具备哪些能力？}
...
class ProviderClient(Protocol):
    """供应商适配器：一个供应商一个实现，负责协议翻译。"""

    def provider_name(self) -> str: ...

    def supports(self, model: str) -> bool: ...

    def chat(self, request: LLMRequest, prompt: RenderedPrompt, route: ModelRoute) -> LLMResponse: ...

    async def stream_chat(
        self, request: LLMRequest, prompt: RenderedPrompt, route: ModelRoute
    ) -> AsyncIterator[LLMChunk]: ...


class LLMGateway(Protocol):
    """业务侧只依赖这一个门面，不直接接触供应商 SDK。"""
...
\begin{itemize}[itemsep=1pt, topsep=2pt]
  \item 业务侧只依赖 \texttt{LLMGateway}，不依赖某个供应商 SDK。
  \item 模型名、供应商、fallback 策略都能配置化。
  \item usage、成本、错误、延迟可以统一记录。
  \item 后续接入新模型，只需要增加 Provider Adapter。
\end{itemize}
...
对外暴露 OpenAI 兼容接口的好处很直接：业务方通常不用大改 SDK，改 \texttt{base\_url} 或网关地址就能从直连供应商切到统一入口。



但这只是入口形状统一，不代表出口也统一。Cloudflare AI Gateway 这类托管网关还要按它当前文档支持的 Provider Native、REST 或 Binding 集成方式接入，不能默认所有供应商都能被当成同一个 OpenAI 协议透传。OpenAI 协议也表达不了一些供应商的专属能力，比如 Anthropic 的 extended thinking、Gemini 的 grounding 元数据。这类能力通常要放进 \texttt{extra\_body}、\texttt{metadata} 或内部扩展字段里，再由 Provider Adapter 转成目标供应商自己的请求格式。
...
{\footnotesize
\begin{tabular}{|p{\dimexpr 0.086\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.286\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.390\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.238\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{维度} & \textbf{OpenAI Chat Completions} & \textbf{Anthropic Messages API} & \textbf{Gemini generateContent} \\
\hline
工具调用字段 & \texttt{tool\_calls} & \texttt{tool\_use} content block & \texttt{functionCall} part \\
\hline
工具结果回传 & \texttt{role=tool} 消息 & \texttt{role=user} + \texttt{tool\_result} content block & \texttt{functionResponse} part \\
\hline
工具 Schema & JSON Schema & JSON Schema 子集 & OpenAPI 子集 \\
\hline
系统提示位置 & \texttt{messages} 中的 system/developer & 顶层 \texttt{system} 字段 & \texttt{systemInstruction} \\
\hline
多工具调用 & 原生支持 & 原生支持 & 结合模型和 SDK 行为单独验证 \\
\hline
专属能力扩展 & \texttt{metadata} / 扩展参数 & thinking、cache\_control 等 & grounding、cachedContent 等 \\
\hline
\end{tabular}
}
...
这里的 \texttt{tier-*} 是网关内部的模型层级名，不是供应商真实模型 ID。生产里通常会由 \texttt{Model Registry} 把 \texttt{tier-fast}、\texttt{tier-balanced}、\texttt{tier-flagship} 映射到当前可用的具体模型，并且在日志里同时记录“模型层级”和“真实模型名”。这样模型升级时只改注册表和灰度配置，不用改业务路由规则。
...
{\footnotesize
\begin{tabular}{|p{\dimexpr 0.276\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.724\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{因素} & \textbf{作用} \\
\hline
\texttt{scene} & 业务场景，决定默认模型和风险等级 \\
\hline
...
class RuleBasedModelRouter:
    ...
    def __init__(
        self,
        route_config_repository: RouteConfigRepository,
        model_health_service: ModelHealthService,
    ) -> None:
        self._route_config_repository = route_config_repository
        self._model_health_service = model_health_service

    def route(self, request: LLMRequest, budget: TokenBudget) -> ModelRoute:
        policy = (
            self._route_config_repository.find_by_scene(request.scene)
            or self._route_config_repository.default_policy()
        )

        for model in policy.candidates:
            if not budget.fits(model):
                continue
            if not self._model_health_service.is_available(model):
                continue
            return ModelRoute.of(model, policy.provider_of(model), policy)

        raise NoAvailableModelError(request.scene)
\end{lstlisting}
...
{\footnotesize
\begin{tabular}{|p{\dimexpr 0.156\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.289\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.556\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{错误类型} & \textbf{是否适合 Fallback} & \textbf{处理方式} \\
\hline
网络瞬断 & 适合 & 短重试后切备用模型 \\
\hline
供应商 5xx & 适合 & 重试 + 熔断 + 切供应商 \\
\hline
429 限流 & 适合但要谨慎 & 读 \texttt{Retry-After}，必要时排队或切模型 \\
\hline
...
幂等键是 LLM 高消费场景的兜底护栏。它和普通 HTTP 幂等不完全一样：HTTP 幂等通常关注“重复执行后资源状态一致”，但 LLM 重复执行会多扣一次钱，而且输出可能变成另一个版本。所以 Gateway 侧不能只存一个“已处理”标记，最好把最终 \texttt{LLMResponse} 也按 \texttt{tenant\_id + idempotency\_key} 缓存下来。重复请求进来时直接返回历史结果，避免重复扣费、重复落库和重复触发工具。
...
\begin{itemize}[itemsep=1pt, topsep=2pt]
  \item 请求 A：输入 500 Token，输出 100 Token。
  \item 请求 B：输入 80K Token，输出 8K Token。
\end{itemize}
...
{\footnotesize
\begin{tabular}{|p{\dimexpr 0.146\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.646\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.208\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{限流维度} & \textbf{控制对象} & \textbf{解决问题} \\
\hline
...
class LLMRateLimiter(Protocol):
    """先 estimate → reserve，调用结束后再用真实 usage reconcile。"""

    def acquire(self, tenant_id: str, user_id: str, model: str, budget: TokenBudget) -> RateLimitPermit: ...

    def reconcile(self, permit: RateLimitPermit, actual_usage: TokenUsage) -> None: ...

    def release(self, permit: RateLimitPermit) -> None: ...
\end{lstlisting}



进入 Gateway 后，先估算 \texttt{input\_tokens + reserved\_output\_tokens}。用户桶、租户桶、模型桶、供应商桶都扣得动，再发请求。扣不动就排队、降级或拒绝，不要先把请求打出去再祈祷供应商别限流。
...
这里更推荐按四步走：\textbf{estimate → reserve → 真实 usage → reconcile}。先用估算值占住预算，调用结束后再用供应商返回的真实 \texttt{usage} 对账修正。不同供应商、不同模型的 tokenizer 和 usage 字段并不完全一致，生产里通常会先用统一近似器扣预算，再用真实 \texttt{input\_tokens}、\texttt{output\_tokens} 修正。如果直接按估算落库，长时间跑下来，成本和配额统计很容易积累出偏差。
...
{\footnotesize
\begin{tabular}{|p{\dimexpr 0.356\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.644\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{字段} & \textbf{说明} \\
\hline
\texttt{request\_id} & 一次业务请求的唯一 ID \\
\hline
\texttt{attempt\_id} & 一次模型调用尝试，fallback 或重试会产生多个 \\
\hline
\texttt{tenant\_id} & 租户或团队 \\
\hline
\texttt{user\_id} & 用户 \\
\hline
\texttt{scene} & 业务场景，比如客服、摘要、代码生成 \\
\hline
\texttt{prompt\_version} & Prompt 版本 \\
\hline
\texttt{provider} & 供应商 \\
\hline
\texttt{model\_tier} & 路由选中的内部模型层级 \\
\hline
\texttt{model} & 实际调用模型 \\
\hline
\texttt{input\_tokens} & 输入 Token \\
\hline
\texttt{output\_tokens} & 输出 Token \\
\hline
\texttt{cached\_tokens} & 命中 Prompt cache 或供应商缓存的 Token \\
\hline
\texttt{cost} & 按价格快照计算的成本 \\
\hline
\texttt{price\_version} & 成本计算使用的价格版本或生效时间 \\
\hline
\texttt{latency\_ms} & 总延迟 \\
\hline
\texttt{ttft\_ms} & 首 Token 延迟 \\
\hline
\texttt{fallback\_used} & 是否发生 fallback \\
\hline
\texttt{error\_code} & 错误类型 \\
\hline
\end{tabular}
}
...
\begin{itemize}[itemsep=1pt, topsep=2pt]
  \item 哪个租户成本最高？
  \item 哪个功能最烧 Token？
  \item 哪个 Prompt 版本导致输出变长？
  \item 哪个模型在某个场景下性价比最好？
  \item fallback 发生在什么时间段、什么供应商、什么模型？
  \item 模型升级后，成本和质量有没有变化？
\end{itemize}


成本计算也要有版本。模型价格、缓存折扣、供应商计费项都会调整，账单对不上时需要知道当时用的是哪份价格表。生产里不要只存一个 \texttt{cost}，最好同时保存 usage 明细、价格版本和计算时间。
...
\begin{itemize}[itemsep=1pt, topsep=2pt]
  \item 元数据长期保留：\texttt{usage}、模型、延迟、成本、\texttt{route\_reason}、错误码这类字段尽量留全。
  \item Prompt 和响应正文采样存储：比如 1\% 全量留 90 天，其他只留 hash、长度、脱敏摘要和结构化错误。
  \item PII 在入口做检测和脱敏：手机号、身份证、银行卡、邮箱、地址等字段先替换为占位符，再进入日志链路。
  \item 按租户配置开关：合同明确允许留存的租户可以保留全量；未明确授权的租户只留元数据。
  \item 提供导出和删除接口：方便满足 GDPR、数据主体请求和企业内部审计要求。
\end{itemize}
...
{\footnotesize
\begin{tabular}{|p{\dimexpr 0.270\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.292\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.202\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.236\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{缓存类型} & \textbf{做法} & \textbf{适合场景} & \textbf{风险} \\
\hline
精确缓存 & 请求完全一致时返回旧结果 & FAQ、固定说明、重复测试 & 个性化和权限场景容易错 \\
\hline
OpenAI Prompt Caching & 稳定长前缀自动命中缓存 & 长系统提示、稳定工具 Schema & 支持模型、阈值和折扣以官方文档和价格表为准 \\
\hline
Anthropic Prompt Caching & 用 \texttt{cache\_control} 标记可缓存块 & 长系统提示、大文档、多轮 Agent & 写入和读取的计费规则要按当前价格表核对 \\
\hline
...
\section*{如何让你设计一个 LLM Gateway，你会怎么做？}


\subsection*{一个生产级 LLM Gateway 长什么样？}
...
{\footnotesize
\begin{tabular}{|p{\dimexpr 0.282\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.718\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{组件} & \textbf{职责} \\
\hline
...
\hline
Provider Adapter & 通过统一的 \texttt{ProviderClient} 接口适配各家协议差异，包括工具调用、流式事件、usage 和错误码 \\
\hline
...
\begin{enumerate}[itemsep=1pt, topsep=2pt]
  \item 统一 API 和 Provider Adapter。
  \item usage、成本、错误和延迟日志。
  \item 规则路由和 Fallback。
  \item Token 预算和租户配额。
  \item 可观测、审计和质量回放。
  \item 轻量分类器或学习型 Router。
\end{enumerate}
...
\subsection*{请求进来后，Gateway 内部怎么跑？}
...
\begin{enumerate}[itemsep=1pt, topsep=2pt]
  \item \textbf{鉴权与租户识别}：确认用户是谁、属于哪个租户、能不能使用当前 AI 功能。
  \item \textbf{判断任务场景}：从接口、业务参数或轻量分类器里得到 \texttt{scene}。
  \item \textbf{渲染 Prompt}：根据场景选择 Prompt 模板，注入用户输入、上下文和工具 Schema。
  \item \textbf{估算 Token 预算}：计算输入 Token，预留最大输出 Token。
  \item \textbf{选择模型和供应商}：根据路由策略、模型状态、预算和风险等级选 primary model。
  \item \textbf{执行限流和预算扣减}：按当前候选模型扣用户、租户、模型、供应商和 Token 桶。
  \item \textbf{调用模型}：通过 Provider Adapter 发起同步或流式请求。
  \item \textbf{解析响应}：处理文本、结构化 JSON、tool call、usage 和 finish reason。
  \item \textbf{失败 Fallback}：按错误类型判断是否重试、切模型、排队或降级。
  \item \textbf{记录 usage 和 trace}：写入成本、延迟、模型、供应商、Prompt 版本和错误信息。
  \item \textbf{返回业务结果}：把统一响应交给业务服务。
\end{enumerate}


\subsection*{路由策略怎么从简单演进到智能？}
...
{\footnotesize
\begin{tabular}{|p{\dimexpr 0.048\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.306\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.323\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.323\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{阶段} & \textbf{对应策略} & \textbf{重点能力} & \textbf{进入下一阶段的信号} \\
\hline
...
阶段二开始做固定规则路由。比如免费用户默认走小模型，企业用户复杂任务走强模型；\texttt{intent\_classification} 走快模型，\texttt{legal\_review} 强制走高质量模型并打上人工审核标记；主模型 429 或 P95 延迟升高时切备用供应商。这个阶段的规则应该尽量可解释，每次路由都写清楚 \texttt{route\_reason}。
...
所以，路由演进有一个很实际的判断标准：\textbf{没有 trace，不上分类器；没有评测集，不上学习型 Router；没有成本上限，不上 Agentic 路由}。先把规则路由、Fallback、usage、\texttt{route\_reason} 跑稳，再谈智能化。
...
{\footnotesize
\begin{tabular}{|p{\dimexpr 0.280\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.720\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{问题} & \textbf{兜底方式} \\
\hline
...
路由日志里一定要记录 \texttt{route\_reason}。不要只记录“用了哪个模型”，还要记录“为什么用它”。
...
没有 \texttt{route\_reason}，路由系统后期会很难调。
...
\subsection*{自研、LiteLLM、Cloudflare AI Gateway、Kong AI Gateway、Inworld Router 怎么选？}
...
{\footnotesize
\begin{tabular}{|p{\dimexpr 0.187\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.288\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.259\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.266\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{方案} & \textbf{主要优势} & \textbf{适合场景} & \textbf{不适合场景} \\
\hline
...
不过，LiteLLM 这类基础设施组件生产使用一定要做版本锁定、镜像固定和供应链扫描。2026 年 3 月，LiteLLM 官方通报过一次 PyPI 发布链路被污染事件，受影响版本包括 \texttt{1.82.7} 和 \texttt{1.82.8}；官方同时说明 GitHub 源码、Docker 镜像和 LiteLLM Cloud 不受影响。这类事件提醒我们：AI Gateway 往往持有大量上游 API Key，一旦依赖链被污染，影响面会比普通业务库更大。
...
这里的顺序不要反：\textbf{先解决工程治理，再追求智能路由}。



\section*{怎么衡量 LLM Gateway 做得好不好？}
...
{\footnotesize
\begin{tabular}{|p{\dimexpr 0.312\columnwidth-2\tabcolsep\relax}|p{\dimexpr 0.688\columnwidth-2\tabcolsep\relax}|}
\hline
\textbf{指标} & \textbf{含义} \\
\hline
...
\begin{itemize}[itemsep=1pt, topsep=2pt]
  \item \href{https://docs.litellm.ai/docs/}{LiteLLM Docs}
  \item \href{https://docs.litellm.ai/blog/security-update-march-2026}{LiteLLM Security Update: Suspected Supply Chain Incident}
  \item \href{https://developers.cloudflare.com/ai-gateway/}{Cloudflare AI Gateway Docs}
  \item \href{https://developers.cloudflare.com/ai-gateway/configuration/request-handling/}{Cloudflare AI Gateway Request Handling}
  \item \href{https://developers.cloudflare.com/ai-gateway/configuration/fallbacks/}{Cloudflare AI Gateway Fallbacks}
  \item \href{https://developers.cloudflare.com/ai-gateway/features/dlp/set-up-dlp/}{Cloudflare AI Gateway DLP}
  \item \href{https://developers.cloudflare.com/ai-gateway/configuration/bring-your-own-keys/}{Cloudflare AI Gateway BYOK}
  \item \href{https://developer.konghq.com/ai-gateway/}{Kong AI Gateway Docs}
  \item \href{https://docs.inworld.ai/router/introduction}{Inworld Router Docs}
  \item \href{https://github.com/ulab-uiuc/LLMRouter}{LLMRouter GitHub Repository}
  \item \href{https://platform.openai.com/docs/guides/prompt-caching}{OpenAI Prompt Caching}
  \item \href{https://docs.anthropic.com/en/docs/build-with-claude/prompt-caching}{Anthropic Prompt Caching}
  \item \href{https://ai.google.dev/gemini-api/docs/caching}{Gemini Context Caching}
\end{itemize}

...
\begin{enumerate}[itemsep=1pt, topsep=2pt]
  \item Harness 是什么，为什么可以把 Agent 理解成 Model + Harness
  \item 为什么同一个模型换一套接口，分数能从 6.7\% 变成 68.3\%
  \item Harness 的六层架构分别解决什么问题
  \item 从零搭 Harness 时，哪些事情应该先做，哪些可以后面再补
  \item OpenAI、Anthropic、Stripe 这些团队到底怎么用 Harness
\end{enumerate}
...
Agent Skills 也可以看成渐进式披露的一种实现。它保留少量元数据，比如名称和描述，详细规则和执行流程只在触发时再加载进上下文。这个思路和 OpenAI 把 \texttt{AGENTS.md} 当目录很接近，只是 Skills 把这个模式标准化了。相关阅读可以看这篇：Agent Skills 详解：是什么？怎么用？和 Prompt、MCP 有什么区别？。
